...
function Heropage() {
  ...
  const [registerFormData, setRegisterFormData] = useState({ username: "", password: "", role: "", name: "" });
    ...
    <section className="relative w-full min-h-screen bg-white overflow-hidden font-sans">
          ...
          <div className="relative">
            ...
            <div className="absolute top-full right-0 mt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
              <div className="bg-gray-800 text-white text-sm font-medium px-4 py-3 rounded-lg shadow-lg whitespace-nowrap">
                <div className="mb-2 pb-2 border-b border-gray-600">{user.name}</div>
                <button
      ...
      {isLoginOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setIsLoginOpen(false)} />
          <div className="relative bg-white rounded-2xl shadow-2xl p-8 w-[90%] max-w-md z-10">
            <button onClick={() => setIsLoginOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition-colors">
            ...
            <form className="space-y-4" onSubmit={handleLogin}>
              ...
              <button type="submit" disabled={isLoading} className="w-full bg-[#FB923C] hover:bg-[#f97316] disabled:bg-[#fbb97a] text-white font-semibold py-3 px-6 rounded-lg shadow-md transition-all duration-300 mt-4 flex items-center justify-center gap-2">
      ...
      {isRegisterOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setIsRegisterOpen(false)} />
          <div className="relative bg-white rounded-2xl shadow-2xl p-8 w-[90%] max-w-md z-10">
            <button onClick={() => setIsRegisterOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
            ...
            <p className="text-center text-gray-600 mb-6">Create a new user account</p>
            {registerError && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded-lg mb-4 text-sm">{registerError}</div>}
            <form className="space-y-4" onSubmit={handleRegister}>
              <input type="text" placeholder="Enter full name" value={registerFormData.name} onChange={(e) => setRegisterFormData({ ...registerFormData, name: e.target.value })} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-[#4DB6B6] transition-colors" required />
              <input type="text" placeholder="Enter username" value={registerFormData.username} onChange={(e) => setRegisterFormData({ ...registerFormData, username: e.target.value })} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-[#4DB6B6] transition-colors" required />
              <input type={showRegisterPassword ? "text" : "password"} placeholder="Enter password" value={registerFormData.password} onChange={(e) => setRegisterFormData({ ...registerFormData, password: e.target.value })} className="w-full pl-10 pr-12 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-[#4DB6B6] transition-colors" required />
              <select value={registerFormData.role} onChange={(e) => setRegisterFormData({ ...registerFormData, role: e.target.value })} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-[#4DB6B6] transition-colors appearance-none bg-white" required>
